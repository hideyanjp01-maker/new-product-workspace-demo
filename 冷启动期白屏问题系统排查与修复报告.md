...
### 步骤0：定义"白屏"判定
- 白屏 = 页面没有 TopBar/SideNav/任何文字，或 DOM 根节点 #root 为空，或 React 崩溃导致未渲染

### 步骤1：路由验证
**状态**：路由配置正确，`/stages/cold-start` 正确指向 `ColdStartStagePage`
- ✅ `src/App.tsx` 路由配置正确
- ✅ 组件导入正确

### 步骤2：添加全局 ErrorBoundary
**新增文件**：
...
**添加的日志**：
```typescript
console.log('[ColdStart] render start')  // 组件函数最顶部（所有 hook 之前）
...
return (
  <MainLayout>
    <PageHeader title="冷启动期" subtitle="产品冷启动和运营管理" backPath="/stages/planning" />
...
### 新增文件（2个）
1. `src/components/ErrorBoundary.tsx` - ErrorBoundary 组件
2. `src/components/ErrorBoundary.css` - 错误边界样式

### 修改文件（2个）
1. `src/main.tsx` - 添加 ErrorBoundary 包裹
2. `src/pages/stages/ColdStartStagePage.tsx` - 添加调试日志、兜底逻辑、CSS调试边框
...
1. **运行时错误**（最可能）
   - ErrorBoundary 会捕获并显示错误信息
   - 查看浏览器 Console 是否有红色报错
...
4. **检查页面显示**：
   ...
   - 如果看到正常内容：问题已解决
   - 如果仍是白屏：检查 DOM（F12 Elements），查看 #root 是否有内容

## 📝 关键 Diff 摘要

### 1. ErrorBoundary 添加（main.tsx）
```typescript
// 修改前
...
### 2. ColdStartStagePage 调试与兜底（ColdStartStagePage.tsx）

**添加的调试代码**：
- `console.log('[ColdStart] render start')` - 组件顶部
- `console.log('[ColdStart] before return', {...})` - return 前
- `style={{ border: '2px solid red', minHeight: 200 }}` - CSS 调试边框
...
### 1. 结论
根因类型待运行时验证，但已添加完整的诊断和兜底机制：
- ✅ ErrorBoundary 捕获运行时错误
- ✅ 调试日志定位执行流程
- ✅ CSS 调试边框验证渲染
- ✅ 兜底逻辑防止崩溃
...
### 2. 证据
- Console 日志输出（需运行时查看）
- ErrorBoundary 错误面板（如果有错误）
- 红色调试边框（验证 CSS）

### 3. 修改文件列表
- 新增：`src/components/ErrorBoundary.tsx`, `src/components/ErrorBoundary.css`
- 修改：`src/main.tsx`, `src/pages/stages/ColdStartStagePage.tsx`

### 4. 修复后验收
- ✅ ErrorBoundary 已添加，错误会显示面板而非白屏
- ✅ 调试日志已添加，可追踪执行流程
- ✅ CSS 调试边框已添加，可验证内容渲染
- ✅ 兜底逻辑已添加，确保不会因数据问题崩溃
- ✅ 强制最小可用渲染，确保始终有内容显示
